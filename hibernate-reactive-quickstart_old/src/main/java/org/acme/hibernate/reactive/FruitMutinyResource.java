...
public class FruitMutinyResource {
  ...
  public Uni<Response> create(Fruit fruit) {
    ...
    return sf.withTransaction((s, t) -> s.persist(fruit)).replaceWith(Response.ok(fruit).status(CREATED)::build);
  }
  ...
  public Uni<Response> update(Integer id, Fruit fruit) {
    ...
    return sf.withTransaction((s, t) -> s.find(Fruit.class, id)
        .onItem()
        .ifNull()
        .failWith(new WebApplicationException("Fruit missing from database.", NOT_FOUND))
        // If entity exists then update it
        .invoke(entity -> entity.setName(fruit.getName()))).map(entity -> Response.ok(entity).build());
  }
  ...
  public Uni<Response> delete(Integer id) {
    return sf.withTransaction((s, t) -> s.find(Fruit.class, id)
        ...
        .call(s::remove)).replaceWith(Response.ok().status(NO_CONTENT)::build);
  }
